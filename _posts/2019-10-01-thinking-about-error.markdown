...
Or have you heard someone saying - "this case should never happen in production because reasons..."?
As a software engineer, I spend a lot of time considering how particular inputs and situations should be handled. Is it an error? It is expected situation?
...
One way to think about a program is like _a_sequence of action_ to achieve some _goal_. For example: compute result of a division of _two variables_;
...
Although to be really useful they all have some flexibility. That is - it is possible to user the same program to display dog picture also. To do that - a program needs to take some _input_. After all, we may like different cats. Or music. Or have different business data. So different users have different inputs and expect different output - most of the time.
...
Now it is all fun and games - when you deal with simple programs designed to [do a one thing][unix-philosophy] and operating with a single user. How about something more challenging: a service that takes input from
...
Surprisingly it actually can get even hairier when we consider sequences across multiple systems - distributed across multiple machines in different data centres.


# Brief overview of errors

When researching a topic of error handling for a proper article, the best I came across was a classification of errors based on when they happen. A syntax, runtime, and logic errors.
This is repeated across countless number of blogs. I found this misleading and most unhelpful. For one - if this is the orderer list - logic error should precede compile-time errors :) You need to make a
...
_Logic errors_ are not computer related per say. It may be a failure to _analyze_ the problem domain. False/noisy data. Or straight _blunder_. And let us not forget that it is possible to commit errors while writing code.
...
And more people review implementation of that specification - more likely the it to be valid.
...
There is also an important dimension to this classification that is not always mentioned. Frequency and severity of the error happening.
...
Input maybe indirectly (command line, config file, script) provided or directly - read from keyboard, mouse, gamepad.
...
The focus of this article is to explore failure of valid program because using QA technique such as testing - it should be possible to determine if a program is valid. That is given a valid input sample it should be possible to establish if
it results in a valid output. That does not guaranty, of course, that we have no bugs and all possible inputs won't uncover bugs. That is why it's important to have representative sample of the input to validate the application.
...
It is also possible to fail to produce an output given valid input and valid output device - if the process failed to secure intermediary resources required to process the input. Out of memory to store the input etc. What makes this one particularly annoying is the fact that - resources required for processing can not always be predicted beforehand as it depends on the input itself. For example if you want to view a raw uncompressed picture 8k picture of a cat - you might need alot of RAM to fit the file in. Or a special app that can handle mem-mapped files.


So plenty of ways to fail. On the up side - understanding where failure can happen is critical to decide what to do about it.


## What do we do when a program fails?
Now that we know where valid bug-free programs fail - what options do we have?
Turns out "it depends on the app". It is one thing when picture viewer app can't find file name because of a type. The app can quit and restart. It is a different case when a server receives a bad request. It is not good idea to close the whole service for other users. What about critical systems where failure is not an option?
...
A service or job queue worker is a different example of the same concept - the input is acquired from some kind of a source is not known beforehand. In this case, input itself can be invalid message. But it should not result in interruption of service.
Indeed it would be a pretty bad service if one user generated a message that brings a service down for everyone.
Thus in the model of message/transaction processing it is possible and beneficial to isolate failures. One can think about isolation as each message/transaction being processed by a separate process. If the input is invalid - good ol'
fail with error message strategy works _AND_ it does not effect other transactions. In fact this model has been implemented in `inetd` and early version of `Apache` web server - where each request has been handled by a new process. The downside is
spawning a new process for each request takes too long and each process has memory overhead. Some improvement of that approach is pre-fork model - where a number of processes created before hand in stand-by mode. When request is received -
it is handed off to a free process. A number of worker-processes can be monitored and when one crasher due to bugs of input issues - a new one spawned in its place.
In order to fight memory overhead of a processes - some serves chose to handle requests in threads. In fact most modern web-servers are threading based.


# Conclusion

In this post, I tried to untangle the mess of error handling. Going beyond simple bugs in implementation, errors can occur in a course of a normal operation. Failures DO HAPPEN and an application must have a strategy to detail with it.
Crashing the app may be a perfectly valid way for simple apps. In case the app does not deal with input acquisition itself, but gets all it needs from the start. In this cases user facing apps can produce: "Invalid input - correct and restart".
The interactive app have an option to maintain a dialogue. You don't want to crush the whole app if you walked a user from a serious of actions only to start again. Maybe just this one input.
